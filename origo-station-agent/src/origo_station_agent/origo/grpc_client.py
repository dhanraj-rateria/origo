"""gRPC implementation of Origo Terrestrial(Ethernet/TCP, mutual TLS,
authenticated with the Origo's own ML-DSA-backed device identity — the same one it uses
for the satellite-facing handshake, not a second local-only credential)."""
...
class GrpcOrigoTerrestrial:
    def __init__(self, *, channel: grpc.aio.Channel, timeout_sec: float = 10.0) -> None:
        self._stub = pb_grpc.GroundOrigoServiceStub(channel)
    ...
    async def verify_and_encapsulate(
        ...
        req = pb.VerifyAndEncapsulateRequest(
            ek=ek, signature=signature, device_id=device_id, nonce=nonce,
        )
        try:
            resp = await self._stub.VerifyAndEncapsulate(req, timeout=self._timeout)
        except grpc.aio.AioRpcError as exc:
            if exc.code() is grpc.StatusCode.INVALID_ARGUMENT:
                raise OrigoRejected(f"Origo rejected ek: {exc.details()}", cause=exc) from exc
            raise OrigoUnavailable(
                f"verify_and_encapsulate failed: {exc.details()}", cause=exc
            ) from exc
        log.info("origo.encapsulated", device_id=device_id, key_id=resp.key_id)
        return EncapsulationResult(
            ciphertext=resp.ciphertext, signature=resp.signature, key_id=resp.key_id,
        )

    async def decrypt_payload(
        self, *, key_id: str, ciphertext: bytes, sequence_number: int,
    ) -> bytes:
        req = pb.DecryptPayloadRequest(
            key_id=key_id, ciphertext=ciphertext, sequence_number=sequence_number,
        )
        try:
            resp = await self._stub.DecryptPayload(req, timeout=self._timeout)
        except grpc.aio.AioRpcError as exc:
            if exc.code() is grpc.StatusCode.INVALID_ARGUMENT:
                raise OrigoRejected(f"Origo rejected ciphertext: {exc.details()}", cause=exc) from exc
    ...
    async def apply_config(self, *, signed_config: bytes) -> None:
        try:
            await self._stub.ApplyConfig(
                pb.ApplyConfigRequest(signed_config=signed_config), timeout=self._timeout,
            )
        except grpc.aio.AioRpcError as exc:
